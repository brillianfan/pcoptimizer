...
class PCOptimizerApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("PC Ultimate Optimizer v2.0")
    ...
    def setup_uninstaller_frame(self):
        ...
        self.refresh_apps_btn = ctk.CTkButton(self.uninstaller_frame, text="Scan Applications", command=self.load_apps)
        self.refresh_apps_btn.grid(row=1, column=0, padx=20, pady=10, sticky="w")
        
        self.apps_scroll = ctk.CTkScrollableFrame(self.uninstaller_frame, width=600, height=300)
        self.apps_scroll.grid(row=2, column=0, padx=20, pady=10, sticky="nsew")
        
        self.uninst_log = ctk.CTkTextbox(self.uninstaller_frame, width=600, height=100, font=ctk.CTkFont(family="Consolas", size=11))
    ...
    def load_apps(self):
        ...
        def task():
            apps = uninstaller.get_installed_apps()
            for app in apps:
                btn = ctk.CTkButton(self.apps_scroll, text=app['name'], anchor="w", fg_color="transparent", 
    ...
    def confirm_uninstall(self, app):
        dialog = ctk.CTkInputDialog(text=f"Type 'YES' to confirm uninstalling {app['name']}:", title="Uninstall Confirm")
    ...
    def setup_sw_health_frame(self):
        self.sw_label = ctk.CTkLabel(self.sw_health_frame, text="Software Update (Winget)", font=ctk.CTkFont(size=24, weight="bold"))
        ...
        self.sw_log = ctk.CTkTextbox(self.sw_health_frame, width=600, height=400, font=ctk.CTkFont(family="Consolas", size=11))
        self.sw_log.grid(row=2, column=0, columnspan=2, padx=20, pady=10, sticky="nsew")
    ...
    def scan_sw_updates(self):
        ...
        self.sw_log.configure(state="disabled")
        threading.Thread(target=lambda: software_health.get_upgradable_apps(self.log_sw), daemon=True).start()
