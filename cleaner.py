...
def run_disk_cleanup(log_callback=None):
    ...
    try:
        # /sagerun:1 triggers the preset configuration (usually needs to be set first with /sageset:1)
        # But we can also just run cleanmgr /autoclean
        subprocess.Popen(['cleanmgr', '/autoclean'])
        if log_callback: log_callback("[OK] Disk Cleanup started in background.")
    except Exception as e:
